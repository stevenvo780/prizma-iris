...
const MessageModal: FC<MessageModalProps> = ({
  ...
  useEffect(() => {
    if (show) {
      setMessageData({
        ...currentMessage,
        order: currentMessage.order || messagesLength + 1,
      });

      if (currentMessage.mediaAttachments) {
        const fileType = getFileType(currentMessage.mediaAttachments);
        setLocalImageUrl(currentMessage.mediaAttachments);
        setHasFileLoaded(true);
        setFileInfo({
          name: getFileNameFromUrl(currentMessage.mediaAttachments),
  ...
  const handleVariableInsert = (variable: string) => {
    const newMessage = messageData.content + variable;
    setMessageData({ ...messageData, content: newMessage });
    handleInputChange({ target: { name: 'message', value: newMessage } });
  };

  return (
    <Modal show={show} onHide={handleClose} size='lg'>
      <Modal.Header closeButton>
        <Modal.Title>Crear Mensaje</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Row className='mb-3'>
          <Col sm={9}>
